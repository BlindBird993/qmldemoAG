...
//    this->_p = Bu*(C^Bv);
  ...
  template<typename T>
  T MyBSurface<T>::_W(int i, int d, T t,const DVector<T>& knot) const
  {
  ...
  template<typename T>
  int MyBSurface<T>::_findIndex(T t,const DVector<T>& knot) const
  {
    int i=_d;
    int n = knot.getDim()-2;
    for(;i<=n;i++){
  ...
  template<typename T>
  void MyBSurface<T>::_makeKnotVectors(DVector<T>& t,int n,T start,T end)
  {
  ...
  template<typename T>
  void MyBSurface<T>::_createLocalSurfaces( PSurf<T, 3>* s, int n1,int n2)
  {
  ...
  template<typename T>
  T MyBSurface<T>::_BDeriv(T t) const
  {
    return 6*(t) - 6*(t*t);
  }
